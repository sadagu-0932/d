...
기본은 두 가지 사건에만 반응합니다 — **먹이(사과)를 먹으면 점수를 얻고, 죽으면 점수를
잃습니다.** 그 위에 "몸이 길수록 더 유리하다"는 신호를 주는 길이 보너스를 두 군데에
얹었습니다.

| 상수 | 값 | 적용 시점 |
|---|---|---|
| `REWARD_FOOD` | `+10` | 먹이를 먹은 스텝 |
| `REWARD_DEATH` | `-10` | 사망한 스텝의 기본값 (벽/몸통 충돌, 타임아웃 불문 — 길이와 무관하게 항상 이 값이 깔림) |
| `LENGTH_BONUS_COEF` | `0.01` | **매 생존 스텝마다**: `+LENGTH_BONUS_COEF * len(snake)` (먹이 섭취 스텝 포함) |
| `DEATH_LENGTH_BONUS_COEF` | `1` | **사망 시에만**: 그동안 먹은 먹이 개수(`score`) 1개당 `+1`, `MAX_DEATH_LENGTH_BONUS`로 상한 |

- 매 생존 스텝의 길이 가산점은 죽지 않은 모든 스텝(먹이를 먹었든 아니든)에 항상 더해집니다.
- 사망 시 보너스는 `min(score * DEATH_LENGTH_BONUS_COEF, MAX_DEATH_LENGTH_BONUS)`만큼 `REWARD_DEATH`에
  더해집니다. `MAX_DEATH_LENGTH_BONUS = REWARD_DEATH * -1 - 5`로 상한이 걸려 있어서,
  **사망 reward는 몸이 아무리 길어도 항상 최소 -5는 남습니다** — 즉 죽는 것 자체가
  이득이 되는 경우는 없습니다.
- 회전 페널티, self-trap 전용 페널티, 빈칸 페널티 등 그 외 조건은 없습니다.
