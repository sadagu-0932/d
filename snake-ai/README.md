...
- **방향 전환(회전) 페널티**: 직진이 아니라 우회전/좌회전으로 움직일 때마다
  `REWARD_TURN_PENALTY = -0.2`만큼 소량 감점합니다(먹이를 먹었어도 예외 없이 적용).
  많이 꺾을수록(지그재그로 움직일수록) 이 감점이 누적되므로, 자주 방향을 트는
  경로를 억제합니다. 좁은 공간에서 계속 방향을 꺾으며 이동하면 자기 몸통으로
  'ㄷ'자(U자) 모양을 만들다가 그 안에 스스로 갇히는 경우가 많은데, 회전 자체에
  비용을 매겨 이런 경로를 덜 선호하게 만드는 것이 목적입니다. 단, 이 페널티는
  `REWARD_FOOD`(10)나 `REWARD_COLLISION`(-20)에 비해 훨씬 작아서, 먹이를 향해
  꼭 필요한 회전이나 위험을 피하기 위한 회전은 여전히 선택됩니다.
