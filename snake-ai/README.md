...
조건을 전부 제거하고, 오직 두 가지 사건에만 반응하는 가장 단순한 형태로 구성했습니다:
**먹이(사과)를 먹으면 점수를 얻고, 죽으면 점수를 잃습니다.** 그 외에는 아무 조건도 없습니다.
...
- 사망 reward는 원인(벽 충돌/몸통 충돌/타임아웃)이나 그 시점의 몸길이와 전혀 상관없이
  항상 `REWARD_DEATH`(-10)입니다.
- 길이 가산점, 빈칸 페널티, 이동 기본 보상, 회전 페널티, self-trap 전용 페널티 등
  과거에 있었던 그 어떤 추가 조건도 없습니다 — 먹이 섭취/사망 외의 모든 스텝은 reward 0입니다.
