...
여러 겹의 보상(길이 보너스, self-trap 전용 페널티, 회전 페널티 등)이 쌓여서 점수 진동 같은
현상의 원인을 구분하기 어려워졌던 이전 버전을 정리하고, 아래 4가지로만 최대한 단순하게
다시 구성했습니다 — 매 스텝의 reward는 항상 이 넷의 조합입니다.

| 상수 | 값 | 적용 시점 |
|---|---|---|
| `REWARD_FOOD` | `+10` | 먹이를 먹은 스텝 |
| `REWARD_DEATH` | `-10` | 사망한 스텝 (벽/몸통 충돌, 타임아웃 불문 — **항상 이 고정값**, 길이와 무관) |
| `LENGTH_BONUS_COEF` | `0.01` | 생존한 모든 스텝마다: `+LENGTH_BONUS_COEF * len(snake)` |
| `EMPTY_PENALTY_COEF` | `0.01` | 생존한 모든 스텝마다: `-EMPTY_PENALTY_COEF * (빈칸 수 / 전체 칸 수)` |

- 사망 reward는 원인(벽 충돌/몸통 충돌/타임아웃)이나 그 시점의 몸길이와 전혀 상관없이
  항상 `REWARD_DEATH`(-10)입니다 — 예전처럼 길이에 비례한 보너스나 self-trap(완전 자기 감금)
  전용 추가 페널티는 없습니다.
- 길이 가산점/빈칸 페널티는 게임오버가 아닌 모든 스텝(먹이를 먹은 스텝 포함)에 매번 함께
  적용됩니다. 빈칸 개수는 `_get_state()`의 flood-fill reachability 계산과는 별개로,
  `GRID_SIZE * GRID_SIZE - len(snake)`로 단순 계산합니다.
- 이동 자체에 대한 별도의 기본 보상(예전의 `REWARD_MOVE`)이나 회전 페널티는 없습니다 —
  길이 가산점과 빈칸 페널티가 매 스텝 보상의 전부입니다.
